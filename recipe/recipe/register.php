...
if (isset($_POST['signup'])) {
    ...
    if(!isset($uname_error)) { //valid uname, check repeation 
		$query = "SELECT * FROM User1 WHERE `uname` ="." '" . $uname . "'";
    ...
    if(!isset($uemail_error)) { //valid uemail, check repeation 
    	$query = "SELECT * FROM User1 WHERE `uemail` ="." '" . $uemail . "'";
    	if($result = mysqli_query($con, $query)){
    ...
    if (!$error) {
//     	echo "INSERT INTO User1(uname, oname, upassword, uemail) VALUES('" . $uname . "', '" . $oname . "', '" . md5(md5($uname).$upassword) . "', '" . $uemail . "')";
    	// $query = INSERT INTO `User1`(`uname`, `oname`, `upassword`, `uemail`) VALUES('" . $uname . "', '" . $oname . "', '" . $upassword . "', '" . $uemail . "')";
        // $quert = "UPDATE `User1` SET `uemail` = 'ian@hotmail.com' WHERE `uname` = 'Ian' LIMIT 1"; //set all the emails if no WHERE, limit one even more than one are set, if no where, at most lose one email
        // mysqli_query($link, $query);
        if(mysqli_query($con, "INSERT INTO User1(uname, oname, upassword, uemail) VALUES('" . $uname . "', '" . $oname . "', '" . md5(md5($uname).$upassword) . "', '" . $uemail . "')")) {
                ...
                <fieldset>
                    ...
                    <div class="form-group">
                        ...
                        <span class="text-danger">
                            <?php if (isset($uname_error)) echo $uname_error."<br/>";
                                    if (isset($sameUname)) echo $sameUname;
                            ?></span>
                    </div>
                    <div class="form-group">
