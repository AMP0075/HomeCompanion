...
// 	print_r($row0);
    if ($row0) {
    	...
    	$errormsg = "You have not editted your profile yet! Please introduce yourself!";
    }
} else {
...
<html>
<head>
    <title>UserInfo | RecipeZila</title>
...
<!-- 	<?php if (isset($errormsg)) {echo $errormsg;} else {echo Hello;}?>  -->
    ...
    <div><?php if (isset($errormsg) or $row0['profile']=='null') {
    ...
    <hr />   
    <h4> <?php echo $name."'s"?> favorite recipes</a></h4> 
<?php
...
//  	echo $query1;
		...
		$colNum = 3; $i = 0;
   		while($row = $result->fetch_assoc()) {
			...
			if (mysqli_num_rows($resultim)==1){
				...
				$iId = "<img src= 'http://127.0.1.1/homecompanion/homecompanion.com/all/recipe/code/Images/".$MyPhoto."'width = 400/>";
			} else {
				$iId = '';
			}
  ...
  <hr />
    <h4> <?php echo $name."'s"?> recipes </a> </h4>
<?php
// 	$query="SELECT * FROM Recipe join RecipeTag natural join Tag WHERE tname ='".$tname."'" ;
	$query2=" SELECT * FROM recipe natural join User1 WHERE uname ='".$name."' order by rTime DESC" ;
...
<?php
	$query2=" select distinct rid, rtitle, rServNum, rCookTime from (SELECT * FROM recipe natural join readtag WHERE rname ='".$name."' order by logtime DESC ) as a limit 5" ;
	$result = mysqli_query($con,$query2);
	$results = mysqli_num_rows($result); 
	$s="<p align='center'>";
   	if ($results>0) {
...
?>    
    ...
    <hr /> 
	<h4 align="center">Want to add a recipe?</h4>
    <form align="center" action = "addRecipe.php">
    <input type="submit" name = "clickhere" value="Click Here!"/>
    </form> </div> 
    
    <hr/>
    <h4 align="center"> <?php echo $name."'s"?> group</h4> 
       <table width="100%">
		<tr>
		<?php
    	$rowCount = 0;
    	...
    	foreach($a as $key => $value) if ($rowCount < $max_recipe) { ?>
     	...
     	<td align="center"><table>
   		<?php } ?>                   
       	<ul>
        	 <?php echo '<a href="group.php?gid=' . $value['gid'] . '">' .$value['gname']. '</a>'."<br>";  ?>
	...
	<hr /> 
	<h4 align="center"> <?php echo $name." joined"?> group</h4> 
       <table width="100%" >
		<tr >
		<?php
    	$rowCount = 0;
	...
	<hr /> 
	<h4 align="center"> <?php echo $name." RSVPed"?> Event</h4> 
       <table width="100%">
		<tr>
		<?php
    	$rowCount = 0;
...
//    		echo $qu;
	...
	<h3 align="center"> Recent events </a> </h3>
	<div align="center">
	<table>
<?php if ($nummt == 0){?>
 <ul>
    No events are added yet.
 </ul>
<?php }else{
    $rowCount = 0;
    $max_recipeM = $nummt;
